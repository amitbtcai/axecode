...
const seedScript = join(
  repoRoot,
  ".agents/skills/interactive-testing/scripts/seed-axecode-smoke-db.mjs",
);
...
try {
  ...
  sessionLaunch: {
    if (launchOnly && args.new !== true) {
      ...
      if (activeSessions.length === 1) {
        ...
        if (health.status === "starting") {
          ...
          console.log(
            `Drive it now: node .agents/skills/interactive-testing/scripts/axecode-cdp.mjs info --session "${existing.sessionFile}"`,
          );
        }
        break sessionLaunch;
      }
    ...
    const appUrl = `http://127.0.0.1:${vitePort}/?axecodeDebugSession=${sessionToken}`;
    ...
    // resolve — only AxeCode's own state stays isolated via AXECODE_BASE_DIR.
    const identityEnv =
      mode === "real"
        ? {}
        : {
            ...(process.platform === "darwin" ? { AXECODE_USE_MOCK_KEYCHAIN: "1" } : {}),
            HOME: homeDir,
            USERPROFILE: homeDir,
    ...
    appProcess = spawn(pnpm.command, pnpm.args, {
      ...
      env: {
        ...process.env,
        AXECODE_DEV_SERVER_PORT: String(vitePort),
        AXECODE_DEV_APP_URL: appUrl,
        AXECODE_CDP_PORT: String(port),
        AXECODE_BASE_DIR: dataDir,
        AXECODE_CDP_USER_DATA_DIR: join(dataDir, "userData"),
        AXECODE_SMOKE_OUT_DIR: outDir,
        AXECODE_DEV_SERVER_REQUIRE_FREE: "1",
        AXECODE_DISABLE_DEVTOOLS: "1",
        ...(launchOnly ? { VITE_AXECODE_SKIP_WELCOME: "1" } : {}),
        ...identityEnv,
      },
    ...
    appProcess.on("exit", (code, signal) => {
      if (!stopping && code !== null && code !== 0 && process.exitCode === undefined) {
        console.error(
          `AxeCode dev process exited with code ${code}${signal ? ` (${signal})` : ""}`,
        );
      }
    });
    ...
    if (launchOnly) {
      ...
      console.log(
        `Drive it without exporting ports: node .agents/skills/interactive-testing/scripts/axecode-cdp.mjs info --session "${sessionFile}"`,
      );
...
async function createFixture() {
  ...
  await writeFile(join(projectDir, "README.md"), "# AxeCode smoke fixture\n");
  ...
  for (;;) {
    ...
    if (request.method === "initialize") {
      result = {
        ...
        capabilities: { tools: {} },
        serverInfo: { name: "axecode-smoke", version: "1.0.0" },
      };
...
`,
  ...
  execFileSync(
    ...
    [
      "-c",
      "user.name=AxeCode Smoke",
      "-c",
      "user.email=smoke@axecode.local",
      "commit",
      "-qm",
      "initial fixture",
    ],
...
async function waitForManagedApp(child) {
  ...
  if (child.exitCode !== null) {
    throw new Error(`AxeCode dev process exited before CDP became ready (exit ${child.exitCode})`);
  }
  const checker = spawn(
    process.execPath,
  ...
  await new Promise((done, reject) => {
    ...
    const checked = (code) => {
      ...
      if (stopping || code === 0) done();
        ...
        reject(
          new Error(
            `AxeCode did not become CDP-ready (exit ${code ?? "unknown"})${detail ? `: ${detail}` : ""}`,
          ),
        );
      }
    };
    const appExited = (code, signal) => {
      ...
      if (stopping) done();
        ...
        reject(
          new Error(
            `AxeCode dev process exited before CDP became ready (exit ${code ?? "unknown"}${signal ? `, ${signal}` : ""})`,
          ),
        );
    };
  ...
  return new Promise((done, reject) => {
    ...
    const exited = (code, signal) => {
      ...
      if (stopping || code === 0) done();
        ...
        reject(
          new Error(`AxeCode dev process exited with code ${code}${signal ? ` (${signal})` : ""}`),
        );
    };
    const cleanup = () => {
