...
const outDir = resolve(
  String(
    args.outDir ??
      process.env.AXECODE_SMOKE_OUT_DIR ??
      join(homedir(), ".axecode-smoke", `integration-${Date.now()}`),
  ),
);
...
function usage() {
  console.error(`Usage:
  node axecode-integration-smoke.mjs audit
  node axecode-integration-smoke.mjs plan [--scope changed|full]
  node axecode-integration-smoke.mjs run [--scope changed|full] [--mode mock|real] [--session <session.json>] [--port <cdp port> --appUrl <dev server url>] [--outDir <dir>] [--ack-manual gate,gate]

Run resolves --session / $AXECODE_DEBUG_SESSION, one active managed debug session for this repo, or a complete explicit port + URL pair. It never guesses ports.`);
}
...
function printPlan(plan) {
  console.log(`AxeCode smoke plan (${plan.scope})`);
...
async function runSmoke(plan) {
  ...
  try {
    ...
    if (plan.automated.includes("browser")) {
      ...
      await evaluate(
        client,
        "window.__axecodeDev.closeSettings(); new Promise((resolve) => setTimeout(resolve, 300))",
        true,
      ).catch(() => undefined);
    }
...
async function baselineScenario(client) {
  const state = await waitForValue(
    () =>
      evaluate(
        ...
        `(() => ({
          ...
          axecodeBridge: typeof window.axecode,
          devBridge: typeof window.__axecodeDev,
          crash: /renderer crash|rendered more hooks/i.test(document.body?.innerText ?? ""),
          welcomeVisible: Boolean(document.querySelector(".axecode-welcome-page")),
    ...
    (candidate) =>
      ...
      candidate.axecodeBridge === "object" &&
  ...
  assert(state.axecodeBridge === "object", "typed preload bridge is missing");
...
async function welcomeDismissalScenario(client) {
  ...
  // Bounded re-click until .axecode-welcome-page is gone (handlers may attach late).
  let clicked = false;
  let dismissed = false;
  for (let attempt = 0; attempt < 5 && !dismissed; attempt += 1) {
    clicked = await evaluate(
      client,
      `(() => {
        const button = document.querySelector(".axecode-welcome-page button");
        if (!(button instanceof HTMLButtonElement)) return false;
        button.click();
        localStorage.setItem("axecode-welcome-seen-v16", "true");
        return true;
      })()`,
    );
    if (!clicked) break;
    await new Promise((resolveWait) => setTimeout(resolveWait, 1_500));
    dismissed = await evaluate(client, `!document.querySelector(".axecode-welcome-page")`);
  }
  ...
  await evaluate(
    client,
    `(() => {
      const app = window.__axecodeDev.stores.app.getState();
...
async function settingsScenario(client) {
  ...
  await evaluate(
    client,
    `window.__axecodeDev.stores.sharedSettings.getState().setMcpServers(${JSON.stringify(configuredMcpServers)})`,
  );
  const sections = [
    "profile",
    "general",
  ...
  for (const section of sections) {
    await evaluate(
      client,
      `window.__axecodeDev.openSettings(${JSON.stringify(section)}); new Promise((resolve) => setTimeout(resolve, 200))`,
      true,
    );
    const state = await waitForValue(
      () =>
  ...
  await screenshot(client, screenshotPath);
  await evaluate(client, "window.__axecodeDev.closeSettings()");
  await evaluate(client, "window.__axecodeDev.stores.sharedSettings.getState().setMcpServers([])");
    ...
    () =>
      evaluate(
        ...
        `(() => {
          ...
          const initialState = window.__axecodeDev.stores.sharedSettings.getState().installedPlugins["browser-tools"];
          return {
  ...
  try {
    ...
    await waitForValue(
      () =>
        evaluate(
          client,
          `window.__axecodeDev.stores.sharedSettings.getState().installedPlugins["browser-tools"]?.enabled === ${JSON.stringify(!detailState.pluginEnabled)}`,
        ),
      Boolean,
    ...
    await evaluate(
      client,
      `(() => {
        const store = window.__axecodeDev.stores.sharedSettings.getState();
        const plugin = Object.values(
          window.__axecodeDev.stores.plugins.getState().pluginsByScope,
        )
          .flat()
        ...
        if (${JSON.stringify(marketplaceState.initialStored)}) {
          window.__axecodeDev.stores.sharedSettings
            .getState()
    ...
    await waitForValue(
      () =>
        evaluate(
          client,
          `(() => {
            const state = window.__axecodeDev.stores.sharedSettings.getState().installedPlugins[${JSON.stringify(pluginId)}];
    ...
    void (async () => {
      ...
      if (message.method === "initialize") {
        result = {
          ...
          capabilities: { tools: {} },
          serverInfo: { name: "axecode-smoke-mcp", version: "1.0.0" },
        };
    ...
    const rendered = await waitForValue(
      () =>
        evaluate(
          ...
          `(() => ({
            text: document.body.innerText,
            viewKind: window.__axecodeDev.stores.app.getState().view.kind,
            settingsOpen: window.__axecodeDev.stores.panel.getState().settingsOpen,
...
async function githubActionsScenario(client) {
  await evaluate(
    client,
    `window.__axecodeDev.openSettings("general"); new Promise((resolve) => setTimeout(resolve, 250))`,
    true,
  );
  const settingsState = await waitForValue(
    () =>
      evaluate(
        client,
        `(() => {
          const settings = window.__axecodeDev.stores.sharedSettings.getState();
  ...
  const opened = await evaluate(
    client,
    `(() => {
      window.__axecodeDev.closeSettings();
      const app = window.__axecodeDev.stores.app.getState();
  ...
  const actionsState = await waitForValue(
    () =>
      evaluate(
        ...
        `(() => ({
          overlayOpen:
            window.__axecodeDev.stores.panel.getState().githubActionsContext !== null,
...
async function controlGeometryScenario(client) {
  ...
  await evaluate(
    client,
    `window.__axecodeDev.openSettings("appearance"); new Promise((resolve) => setTimeout(resolve, 250))`,
    true,
  );
...
async function threadSearchScenario(client) {
  await evaluate(
    client,
    `window.__axecodeDev.stores.panel.setState({ threadSearchOpen: true }); new Promise((resolve) => setTimeout(resolve, 80))`,
    true,
  );
  const state = await waitForValue(
    () =>
  ...
  await screenshot(client, screenshotPath);
  await evaluate(client, "window.__axecodeDev.stores.panel.setState({ threadSearchOpen: false })");
  return { ...state, screenshotPath };
}
...
async function browserScenario(client) {
  ...
  const result = spawnSync(
    process.execPath,
    [
      join(scriptDir, "axecode-browser-smoke.mjs"),
...
async function runMockIntegrations(report, client, gates) {
  const fixture = await evaluate(
    client,
    `(() => {
      const state = window.__axecodeDev.stores.app.getState();
      const project =
      ...
      return {
        ...
        bridgeKeys: Object.keys(window.axecode),
        bodyText: document.body.innerText,
      };
    })()`,
  );
...
async function runMockGate(client, gate, fixture) {
  switch (gate) {
    ...
    case "native-auth-update": {
      await evaluate(
        client,
        `window.__axecodeDev.setUpdate({ phase: "downloaded", version: "mock-smoke" })`,
      );
      const update = await evaluate(client, "window.__axecodeDev.stores.update.getState()");
    ...
    case "provider-live": {
      const state = await evaluate(
        ...
        `(() => {
          ...
          window.__axecodeDev.stores.agentStatuses.getState().hydrateFromCache({
            windows: [candidate],
            wsl: [],
          });
          window.__axecodeDev.stores.app.getState().openDraft(${JSON.stringify(fixture.project.id)});
...
async function bridgeInvoke(client, method, payload) {
  ...
  return evaluate(client, `window.axecode[${JSON.stringify(method)}](${payloadText})`, true);
}

async function resetDrivenState(client) {
  await evaluate(client, "window.__axecodeDev?.reset()");
}
...
async function waitForTarget() {
  ...
  if (cdpRespondedWithPages) {
    throw new Error(
      `no AxeCode CDP target matching ${appUrl} on port ${port}. ` +
        `Available page targets: ${lastPageUrls.join(", ")}. ` +
        `Check AXECODE_APP_URL / port allocation.`,
    );
  }
  throw new Error(`no AxeCode CDP target at ${appUrl} on port ${port}`);
}
...
function printReport(report, reportPath) {
  console.log("\nAxeCode integration smoke report");
