...
use axecode_computer_use::backend::linux::LinuxBackend;
...
use axecode_computer_use::protocol::window::{WindowRef, WindowSource};
...
fn sends_background_events_and_captures_an_xwayland_window() {
    ...
    match backend.capture(&resolved, &CancelToken::default()) {
        ...
        Err(error) => {
            assert_eq!(
                error.code,
                axecode_computer_use::protocol::ErrorCode::CaptureFailed
            );
