# AxeCode computer-use helper

This crate is AxeCode's bundled native host for background desktop inspection and control on Windows, macOS, and Linux. It communicates only through newline-delimited JSON on stdin/stdout; diagnostics go to stderr.
...
```bash
axecode-computer-use --state-dir /path/to/private/app-state
```

Send one request per line:

```json
{ "id": 1, "action": "hello", "input": { "protocolVersion": 1, "clientVersion": "axecode" } }
```

The response is either `{ "id", "ok": true, "result" }` or `{ "id", "ok": false, "error", "code" }`. Responses may be out of order. `cancel` cooperatively cancels an active request; `shutdown` closes the host. Run `axecode-computer-use --hello` for a one-shot capability/permission handshake or `--version` for the helper version.
...
| macOS                | Finder, TextEdit, Safari, Terminal, Chrome, Brave, an Electron app                            | With TCC grants absent, verify the helper reports permission denial without opening a prompt. After granting AxeCode access manually, verify element actions leave the window order alone, that background keyboard input to a window its app has not focused raises it inside that app and reports `in_app_focus_changed` while the frontmost app keeps the user's focus, the active keyboard layout drives shortcuts, ScreenCaptureKit captures an occluded window, and foreground uses the takeover border. On Brave and the Electron app, confirm the page appears in the tree with actionable elements, a background coordinate click reports `coordinate_resolved_by_tree` and presses the control under the point, and a background coordinate scroll is refused rather than dropped. Confirm a `mode:"background"` `launch_app` leaves the frontmost app unchanged, and that a locked screen refuses foreground with `screen_locked`, returns no screenshot with a `capture_failed` note, and yields only the app-proxy accessibility tree. |
