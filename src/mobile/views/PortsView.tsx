...
export function PortsView() {
  ...
  function describeError(error: unknown): string {
    if (error instanceof RemoteClientError) {
      if (error.status === 404) {
        return t`Update Axe Code on your desktop to use port forwarding.`;
      }
   ...
   * can reach the forward from here, so surface the same "update Axe Code"
  ...
  function openForwardTarget(enterPath: string | undefined, listenPort: number): void {
    ...
    toast.warning(t`Update Axe Code on your desktop to use port forwarding.`);
  }
