...
test -f "$ARCHIVE" || { printf 'Uploaded Axe Code runtime archive was not found.\n' >&2; exit 43; }
...
case "$CONNECTION_ID" in
  *[!0-9a-f-]*|'') printf 'Invalid Axe Code SSH connection id.\n' >&2; exit 2 ;;
esac
...
test -f "$RUNTIME/.ready" || { printf 'Axe Code remote runtime is not installed.\n' >&2; exit 45; }
...
if [ -n "$PID" ] && [ -n "$PORT" ] && [ "$CURRENT_RUNTIME" = "$RUNTIME_HASH" ] && kill -0 "$PID" 2>/dev/null && server_ready "$PORT" "$APP_VERSION"; then
  ...
  if [ "$READY" -ne 1 ]; then
    printf 'Axe Code Helper failed to start or returned an incompatible protocol.\n' >&2
