...
function ModelVisibilityRow(props: {
  ...
  if (item.type === "header-provider") {
    ...
    return (
      <div
        ...
        tabIndex={0}
        className="axecode-menu-item group mx-1.5 mb-1 mt-2 flex h-8 cursor-default items-center gap-2 border-b border-border bg-overlay px-2 text-sm font-semibold text-foreground first:mt-0"
  ...
  if (item.type === "header-sub") {
    ...
    return (
      <div
        ...
        aria-checked={state === "all" ? "true" : state === "none" ? "false" : "mixed"}
        tabIndex={0}
        className="axecode-menu-item group mx-1.5 flex h-6 cursor-default items-center pl-4 pr-2 text-[10px] font-normal italic tracking-normal text-muted/60"
        onClick={handleToggle}
        onKeyDown={(event) => {
          if (event.key === "Enter" || event.key === " ") {
            event.preventDefault();
  ...
  return (
    <div
      ...
      tabIndex={0}
      className={`axecode-menu-item group mx-1.5 flex h-7 cursor-default items-center text-foreground ${
        indent ? "pl-4" : ""
      }`}
