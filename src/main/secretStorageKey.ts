...
function createPersistentKey(path: string): string {
  ...
  try {
    ...
    throw new Error("Unable to encrypt the AxeCode secret storage key.");
  }
  try {
    writeFileAtomic(path, encrypted.toString("base64"), { encoding: "utf8", mode: 0o600 });
  } catch {
    throw new Error("Unable to persist the encrypted AxeCode secret storage key.");
  }
  return key;
}
...
export function readOrCreateSafeStorageSecretKey(
  ...
  try {
    ...
    throwSecretStorageError("Unable to read the encrypted AxeCode secret storage key.");
  }
