...
function buildWatchPrompt(watch: PrWatch, details: PrDetails, signals: WatchSignals): string {
  ...
  return [
    `AxeCode is watching pull request #${watch.prNumber} (${details.title}) on branch "${watch.headBranch}".`,
    ...
    "All currently reported checks have completed. Inspect their final results, but do not run long-lived watch or polling commands such as `gh run watch`; after pushing, exit so AxeCode can recheck the PR and handle further repairs or auto-merge.",
    "Do not merge the PR; AxeCode handles auto-merge separately. If no code change is needed, explain why and leave the repository untouched.",
    "",
