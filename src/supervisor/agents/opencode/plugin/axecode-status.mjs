/**
 * OpenCode plugin forwarder for AxeCode thread status.
 *
 ...
 *
 * Filename note: deliberately `axecode-status.mjs` in the source tree.
 * The supervisor drops it as `axecode-status.js` into OpenCode's
 ...
 * `axecode-status.js` produces "axecode-status". The Windows panel
 ...
 * builds will read it.
 *
 * Safe outside AxeCode: when the env vars are missing the handlers no-op.
 */
...
//      deployed plugin file is `axecode-status.js` and its manifest is
//      `axecode-status.plugin.json`.
...
//      staged file is `axecode-status.mjs`.
...
const CROSSAGENT_SESSION_ID_ARG = "__axecode_provider_session_id";

function injectCrossagentSessionId(input, output) {
  if (process.env.AXECODE_OPENCODE_SESSION_ROUTING !== "1") return;
...
function debugLog(message) {
  if (hookDebugEnabled()) {
    process.stderr.write(`[axecode-opencode] ${message}\n`);
  }
}

// `event.type` → AxeCode intent. `permission.updated` and the (per-docs but
...
async function forwardIntent(eventType, intent, sessionId, extra) {
  ...
  const agentKind = process.env.AXECODE_AGENT_KIND ?? "opencode";
  const supervisorProtocol = Number(process.env.AXECODE_HOOK_PROTOCOL_VERSION ?? PROTOCOL_VERSION);
  const negotiatedProtocol = Math.min(PROTOCOL_VERSION, supervisorProtocol || PROTOCOL_VERSION);

  if (!url || !secret) {
    debugLog(`skip ${eventType}: missing AXECODE_HOOK_URL or AXECODE_HOOK_SECRET`);
    return;
  }
