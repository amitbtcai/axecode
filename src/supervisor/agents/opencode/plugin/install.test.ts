...
function makeBaseDir(): string {
  const dir = mkdtempSync(join(tmpdir(), "axecode-opencode-plugin-"));
  tempDirs.push(dir);
  return dir;
}
...
describe("getOpenCodePluginPaths", () => {
  it("places axecode-managed staging under the supplied base dir", () => {
    ...
    expect(paths.opencodePluginFile).toBe(
      join(process.env.OPENCODE_CONFIG_DIR, "plugins", "axecode-status.js"),
    );
  });
  ...
  it("uses ~/.config/opencode/plugins when OPENCODE_CONFIG_DIR is unset", () => {
    ...
    delete process.env.OPENCODE_CONFIG_DIR;
    const paths = getOpenCodePluginPaths({ envKind: "posix", baseDir });
    expect(paths.opencodePluginFile.endsWith(join("plugins", "axecode-status.js"))).toBe(true);
  });
});
...
describe("installOpenCodePlugin", () => {
  it("stages assets and drops the plugin + manifest into OpenCode's plugins dir", () => {
    ...
    // AxeCode-side staging
    expect(existsSync(join(result.paths.pluginDir, "plugin.json"))).toBe(true);
    expect(existsSync(join(result.paths.pluginDir, "axecode-status.mjs"))).toBe(true);

    // OpenCode-side drops: .js plugin file (auto-discovered) + sibling manifest
    const droppedPlugin = join(opencodeDir, "plugins", "axecode-status.js");
    const droppedManifest = join(opencodeDir, "plugins", "axecode-status.plugin.json");
    ...
    expect(
      readFileSync(join(result.paths.pluginDir, "axecode-status.mjs")).equals(
  ...
  it("overwrites private Crossagents routing metadata from the trusted tool context", async () => {
    ...
    const originalRouting = process.env.AXECODE_OPENCODE_SESSION_ROUTING;
    process.env.AXECODE_OPENCODE_SESSION_ROUTING = "1";
    try {
      const output = { args: { __axecode_provider_session_id: "forged", prompt: "review" } };
      await hooks["tool.execute.before"]?.(
        { tool: "crossagents_run_agent", sessionID: "session-trusted" },
        output,
      );
      expect(output.args.__axecode_provider_session_id).toBe("session-trusted");
      ...
      if (originalRouting === undefined) delete process.env.AXECODE_OPENCODE_SESSION_ROUTING;
      else process.env.AXECODE_OPENCODE_SESSION_ROUTING = originalRouting;
    }
  });
  ...
  it("scrubs a stale `file://` plugin entry that older builds left in opencode.json", () => {
    ...
    writeFileSync(
      ...
      JSON.stringify({
        ...
        plugin: [
          ...
          `file:///prior/baseDir/agent-plugins/opencode/axecode-status.mjs`,
        ],
      }),
      "utf8",
    );
    ...
    // AxeCode entry removed; user's other plugin entry is preserved.
  ...
  it("removes the `plugin` key entirely when ours was the only entry", () => {
    ...
    writeFileSync(
      configPath,
      JSON.stringify({
        plugin: [`file:///prior/baseDir/agent-plugins/opencode/axecode-status.mjs`],
      }),
      "utf8",
    );
  ...
  it("preserves user MCP entries whose names match AxeCode built-ins", () => {
    const baseDir = makeBaseDir();
    const opencodeDir = makeBaseDir();
    process.env.OPENCODE_CONFIG_DIR = opencodeDir;
    const configPath = join(opencodeDir, "opencode.json");
    const userMcp = Object.fromEntries(
      ["browser", "crossagents", "chrome", "computer_use", "axecode"].map((name) => [
        name,
  ...
  it("restores entries shadowed by the retired MCP config projection", () => {
    ...
    process.env.OPENCODE_CONFIG_DIR = opencodeDir;
    const configPath = join(opencodeDir, "opencode.json");
    const sidecarPath = join(opencodeDir, ".axecode-managed-mcp.json");
    const originalBrowser = { type: "local", command: ["user-browser"] };
    writeFileSync(
      configPath,
      JSON.stringify({
        mcp: {
          browser: { type: "remote", url: "http://axecode/browser" },
          axecode_only: { type: "remote", url: "http://axecode/only" },
        },
      }),
      "utf8",
    );
    writeFileSync(
      sidecarPath,
      JSON.stringify({ browser: originalBrowser, axecode_only: null }),
      "utf8",
    );
  ...
  it("removes a legacy axecode-status.mjs left behind by an older install", () => {
    const baseDir = makeBaseDir();
    const opencodeDir = makeBaseDir();
    process.env.OPENCODE_CONFIG_DIR = opencodeDir;

    const legacyPath = join(opencodeDir, "plugins", "axecode-status.mjs");
  ...
  it("treats a missing dropped manifest as not-installed", () => {
    ...
    unlinkSync(join(opencodeDir, "plugins", "axecode-status.plugin.json"));
  ...
  it("removes dropped files, staging dir, and the opencode.json entry", () => {
    ...
    writeFileSync(
      ...
      JSON.stringify({
        plugin: [
          "@user/other-plugin",
          `file://${result.paths.pluginDir.replace(/\\/g, "/")}/axecode-status.mjs`,
        ],
    ...
    expect(existsSync(join(opencodeDir, "plugins", "axecode-status.plugin.json"))).toBe(false);
